# Tests removed: dlw_store_release() does not exist in the package.
...
if (FALSE) {
  ...
  test_that("Usage with update_inventory_list=FALSE", {
    ...
    release_file <- file.path(
      local_dir,
      "_release",
      "pip_raw_inventory_RELEASE_LABEL.qs"
    )
  ...
  test_that("skip_err=TRUE => saving inventory list fails => we skip & return NULL", {
    ...
    expect_match(
      pip_logs[[idx]],
      "Could not save master list ''|No valid file path specified"
    )
  })
} # end if (FALSE)
