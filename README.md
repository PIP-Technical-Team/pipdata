...
| Function | Purpose |
|---|---|
| `pd_process_data()` | Main entry point: iterate over the DLW inventory, merge auxiliary data, clean variables, validate, and save |
...
The dependency layer separates `clean` (`survey_id`), `metadata` (`pip_id`),
and `deflate` (`pip_id`) work. Stamp remains authoritative for immutable
artifact versions; pipdata stores a rebuildable, release-scoped provenance
manifest. Legacy state is never rebuilt implicitly: inspect
`pd_change_report()`, then use explicit `bootstrap = TRUE` and a restrictive
...
storage and complete the signed Windows/SMB fencing and unique-rename smoke
test documented in `inst/doc/staged-dependency-manifest.md`. Local cache
defaults are development-only and do not indicate production activation.
