...
- **stamp round-trip strips `*_data_level` columns and scalar attribute structure**: `vars_to_attr()` moves `ppp_data_level`, `cpi_data_level`, and `pop_data_level` from columns into object attributes before `pip_write()`. After `pip_read()` they exist only as attributes, not columns. Any code loading a stamp-saved survey and needing those values as columns must call `restore_data_level_cols()` (in `R/pd_deflation.R`) first. Similarly, scalar attributes such as `survey_id` are plain character strings after a stamp round-trip — never access them as `attr(dt, "survey_id")$values` (the `$values` wrapper is only present on the in-memory pipeline object, not after deserialization). See `.cg-docs/solutions/bugs/2026-05-05-data-level-columns-stripped-on-stamp-round-trip.md`.
