...
- **Typed stage boundary**: Internal `pipeline_context` and
  `pipdata_stage_result` S3 lists expose the unchanged C2 dependency context,
  accepted plan identity, compact unit outcomes, stable condition records, and
  finalized artifact references. C2 remains authoritative for actions, hashes,
  receipts, currentness, and manifests; B2 `pipdata_log` remains the sole
  persistent log. Portable RDS-v3 projections exclude runtime environments,
  inventories, household data, raw conditions, and log rows. Deflation uses a
  shared core: the public wrapper still returns the master inventory, while the
  internal typed entry point returns the stage result. Survey-domain failures
  may continue; lease, exact-hash, manifest, receipt, checkpoint, and other
  integrity failures stop later writes. This interface does not activate
  production: signed Windows/SMB fencing and immutable unique-rename evidence
  remain required.
