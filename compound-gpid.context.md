...
- **`*_data_level = "area"` is a column pointer, not a level name**: When `ppp_data_level`, `cpi_data_level`, or any `*_data_level` attribute equals `"area"`, it means "resolve to the per-row values of the `area` column in the data.table." It is **not** a literal level name. Deflation helpers (`add_rep_lvl()`, `add_ppp()`, `add_cpi()`) must check for this case and use `dt$area` as per-row lookup keys into named CPI/PPP vectors (whose keys are `"rural"`, `"urban"`, `"national"`). `"national"` is the only literal level value stored directly. Surveys with subnational PFW domains (reporting_level == 2, cpi_domain_var == "urban") always receive `*_data_level = "area"` from `add_dom_vars()`. See `.cg-docs/solutions/bugs/2026-05-06-subnational-deflation-area-attribute-not-resolved.md`.
