...
```{r}
lf_wbpip <- pd_wbpip_clean(lf_dlw, cpfw )
# names(lf_wbpip)
```

These functions are built in a way that they don't skip the code if there is an error. Rather, the error is  stored for later analysis. 

```{r no-error}
# Miss one argument on purpose
lf_wbpip_error <- pd_wbpip_clean(lf_dlw)
lf_wbpip_error
```
