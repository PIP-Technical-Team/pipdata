#' Retrieve and Pin GMD Catalog Datasets to a Local Directory
#'
#' @description
#' This wrapper function automates the process of managing GMD catalog datasets by performing the following tasks:
#' 1. Checks for new datasets in the GMD catalog using the inventory file (`dlw_gmd_inv`), which contains the current list of available GMD datasets.
#' 2. If new datasets are found (specifically `"GPWG"`, `"GROUP"`, `"BIN"`, `"HIST"`, `"ALL"`, `"ASPIRE"`, and `"L"`), downloads them using `dlw::dlw_get_gmd` and pins them to the local directory.
#' 3. Updates the inventory file (`dlw_gmd_inv`) with information about the newly downloaded datasets.
#'
...
pipdata_get_gmd <- function(
  ...
  # initialize working release and dlw inventory and data boards
  pipfun::get_wrk_release(verbose = FALSE)
  dlw_data_board <- pipfun::get_pins_boards(board = "dlw_data")
  dlw_inv_board  <- pipfun::get_pins_boards(board = "dlw_inventory")

  # check directory existence for inventory and data boards
  ...
  inv_gmd <- dlw_gmd_new(inv_gmd_list = inv_gmd_list,
  ...
  cli::cli_alert_info("pin folder: {.dir {dlw_data_board$path}}")
  ...
  # get list of datasets already pinned in the local folder
  ...
  cli::cli_alert_success("GMD inventory file is saved at: {.dir {dlw_inv_board$path}}")

  if (log) {

    pipfun::log_add("info", "Inventory file is saved",
                    name = "pipdata_log",
                    logmeta = list(saved_at = dlw_inv_board$path))
  }
  ...
  if (save_log & log) {
    ...
    pipfun::log_add("info", "logging file is saved",
                    name = "pipdata_log",
                    logmeta = list(log_path = "dlw_gmd_log"))
