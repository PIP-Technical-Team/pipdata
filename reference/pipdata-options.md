...
`pipdata` is controlled by a small set of
[`options()`](https://rdrr.io/r/base/options.html). Each option has a
...
`getOption("pipdata.verbose", default = TRUE)`. You can still override
it per-call by passing `verbose = FALSE` (or `TRUE`) explicitly — the
